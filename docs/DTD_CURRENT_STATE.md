...
- **Risk/uncertainty:** this is active-credential exposure in a historical artifact, not merely stale configuration. The source bucket has no public IAM member, but individual IAM access and historical access remain unresolved; no matching Data Access record was found, and project-level Data Access logging was not configured. This does not establish public exposure or that the file entered the container image. No credential value is reproduced in this record; that reporting hygiene is not evidence that no exposure occurred.
...
- **P0 rotation scope (identity/scope only):** the archived values match enabled version 1 of `dtd-abr-guid` (ABR verification), `dtd-admin-pass` (`/ops` authentication and fallback signing), `dtd-places-api-key` (Places API/billing), `dtd-mongo-url` (Atlas database access), `dtd-resend-api-key` (transactional email), `dtd-sentry-dsn` (event ingestion), `dtd-stripe-secret-key` (Stripe API operations), `dtd-stripe-webhook-secret` (Stripe webhook authenticity), and `dtd-trainer-action-token-secret` (trainer/billing action-link signing). The current service consumes `latest` for these; all are version 1 except MongoDB, whose archived version is 1 while the service uses version 4. The archive also contains a Gemini API key and Stripe test-secret key not injected by the current service, plus a Stripe test publishable key (public identifier, not a rotation secret). No values are recorded here.
- **Next action:** execution is not authorised. First replace the Scheduler shared-secret boundary under DF-002, then treat the listed archive credentials and the non-injected Gemini/test secret keys as compromised and rotate/revoke them provider by provider, verify each dependent flow, and only then decide archive retention or removal after rollback impact is understood.
...
- **Planning status:** an identity-based replacement sequence is approved, but no production change is authorised. Because this Cloud Run service also exposes public routes, the replacement must validate Scheduler's signed OIDC token and expected service-account identity/audience at this one internal endpoint; a service-wide IAM gate would break public API traffic.
- **Next action:** retain the warning job; create the dedicated Scheduler identity and route-level OIDC validation, configure the job to send that token, remove the shared credential and duplicate header, then verify the next authenticated run. The concrete implementation/verification plan must be reviewed before execution.
...
- **Local repair status:** `GET /api/seo/{slug}` now rejects non-canonical and nested slugs before any provider call or database write. Canonical pages return a non-indexable, read-only navigation response until both `SEO_MIN_PUBLISHED_TRAINERS=3` and `SEO_MIN_CONTENT_WORDS=500` are configured and the locality meets both gates; only then can content be generated and persisted. Static `robots.txt` and a root-only sitemap are present in the frontend build source and were deployed successfully ahead of Firebase's SPA fallback. The backend SEO endpoint repair, its production configuration, and any existing-page migration remain unverified in production.
- **Threshold decision:** 24 September 2026 — the owner approved three eligible published trainers and 500 content words as adjustable starting thresholds. They are set in the environment template and specification; the live API remains fail-closed until a separately approved backend release applies them.
