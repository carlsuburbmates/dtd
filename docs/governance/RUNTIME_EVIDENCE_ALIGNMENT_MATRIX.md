...
- visibility-first `/ops` with bounded Layer 1 review-state persistence
...
- Phase 3 owner review is complete under the current governance scope
- remaining gates are deferred actual-domain activation proofs before final public launch approval
...
|---|---|---|---|---|---|---|---|---|---|
| Public live matching exposure is runtime-controlled | `DTD_PROJECT_CONTEXT.md`, `LAUNCH_GATE.md`, `LOCK_STATE.md` | Home exposure must be explicitly gated and consistent across backend, frontend, tests, and public copy | `backend/server.py` defines `PUBLIC_MATCHING_ENABLED`, exposes it from `GET /api/config`, gates `POST /api/match` and `POST /api/intros`; `frontend/src/pages/Home.jsx` consumes `/config`; `backend/tests/test_public_mode_unit.py` asserts config and gate behavior | `docs/governance/LOCK_STATE.md`, `docs/governance/EXECUTION_STATUS.md`, `docs/COMPLETE_WEBSITE_PAGE_SPEC.md` | Implemented | Low | Keep as-is and preserve doc/runtime sync | Normal Ops for monitoring; Technical-Owner Mode for changes | No |
| Launch phase remains separate from matching exposure | `DTD_PROJECT_CONTEXT.md`, `BUILD_CHECKLIST.md`, `LAUNCH_GATE.md`, `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md` | Runtime should represent launch phase separately from `PUBLIC_MATCHING_ENABLED` | `PUBLIC_LAUNCH_PHASE` env var read at `backend/server.py:104`; `_get_or_create_launch_phase_state()` persists phase state to `system_state` at `server.py:832-852`; oversight exposes `launch_phase_state` at `server.py:2406`; `/ops` surfaces phase separately from matching gate | Docs repeatedly state the separation rule, especially `LOCK_STATE.md`, `EXECUTION_STATUS.md`, `BUILD_CHECKLIST.md` | Implemented | Low | Keep as-is | Normal Ops for monitoring; Technical-Owner Mode for changes | No |
| Current approved posture is supply-first with public matching off | `LOCK_STATE.md`, `EXECUTION_STATUS.md`, `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md` | Runtime should make the current approved posture inspectable in product-backed evidence | `launch_phase_state` persisted with `current_phase=supply_first`, `public_matching_enabled=false`, `public_emphasis=waitlist_first` at `server.py:810-852`; exposed in oversight and `/ops` | `docs/governance/LOCK_STATE.md`, `docs/governance/EXECUTION_STATUS.md` | Implemented | Low | Keep as-is | Normal Ops for reading; Technical-Owner Mode for changes | No |
...
| Requirement | Source authority | Required runtime behaviour | Current code/runtime evidence | Current docs/evidence reference | Status | Risk if unresolved | Required next action | Suggested owner mode | Blocks runtime/evidence alignment? |
|---|---|---|---|---|---|---|---|---|---|
| Supply readiness is visible from product-backed evidence | `DTD_PROJECT_CONTEXT.md`, `LAUNCH_GATE.md`, `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md`, `INITIAL_LAUNCH_EVIDENCE_MODEL_SUPPLY_FIRST.md` | `/ops` should show enough product-backed data to judge supply readiness without DB inspection | `backend/server.py` exposes `phase_readiness_snapshot`, `kpi_prelaunch`, `submissions_summary`, `integrity`, and `reactivation_summary`; `frontend/src/pages/Ops.jsx` overview renders readiness status, recommendation, and blockers alongside supply metrics | `docs/governance/EXECUTION_STATUS.md`, `docs/INITIAL_LAUNCH_EVIDENCE_MODEL_SUPPLY_FIRST.md`, `docs/governance/LOCK_STATE.md` | Implemented | Low | Keep as-is | Normal Ops for review; Technical-Owner Mode for changing underlying policy | No |
...
| Geography coverage is inspectable for supply decisions | `LAUNCH_GATE.md`, `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md`, `INITIAL_LAUNCH_EVIDENCE_MODEL_SUPPLY_FIRST.md` | Supply evidence should expose target geography coverage | `backend/server.py` now exposes `ops_supply_geography`, including top trainer suburbs, waitlist suburbs, demand gaps, and coverage counts; `frontend/src/pages/Ops.jsx` renders these in the overview and trainer-supply decision surfaces | `docs/governance/LOCK_STATE.md`, `docs/governance/OPERATIONS_CONSOLE_SPEC.md`, `docs/governance/WORKFLOW_SURFACE_MATRIX.md`, `docs/design/WIREFRAME_STATE_MAP.md` | Implemented | Low | Keep current contract aligned | Normal Ops | No |

## 4. Trainer Acquisition And Activation

| Requirement | Source authority | Required runtime behaviour | Current code/runtime evidence | Current docs/evidence reference | Status | Risk if unresolved | Required next action | Suggested owner mode | Blocks runtime/evidence alignment? |
|---|---|---|---|---|---|---|---|---|---|
| Trainer acquisition path stays open during supply-first launch | `DTD_PROJECT_CONTEXT.md`, `BUILD_CHECKLIST.md`, `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md` | Trainer onboarding remains available even when public live matching is gated | Routes exist in `frontend/src/App.js`; `Home.jsx` always links to `/trainers`; `PUBLIC_MATCHING_ENABLED` gates owner match/contact path only | `docs/COMPLETE_WEBSITE_PAGE_SPEC.md`, `docs/governance/LOCK_STATE.md` | Implemented | Low | Keep as-is | Normal Ops | No |
...
| Founder can see trainer acquisition and activation trend in `/ops` | `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md`, `INITIAL_LAUNCH_EVIDENCE_MODEL_SUPPLY_FIRST.md` | `/ops` should expose trend and usable supply, not just route existence | `backend/server.py` now exposes `ops_supply_trends`, including submission pace, published pace, intro-ready now, blocked now, and reactivation motion; `frontend/src/pages/Ops.jsx` renders trend-backed supply cards in overview and trainer supply | `docs/governance/OPERATIONS_CONSOLE_SPEC.md`, `docs/governance/WORKFLOW_SURFACE_MATRIX.md`, `docs/design/WIREFRAME_STATE_MAP.md` | Implemented | Low | Keep current contract aligned | Normal Ops | No |

## 5. Passive Owner Waitlist

| Requirement | Source authority | Required runtime behaviour | Current code/runtime evidence | Current docs/evidence reference | Status | Risk if unresolved | Required next action | Suggested owner mode | Blocks runtime/evidence alignment? |
|---|---|---|---|---|---|---|---|---|---|
| Passive owner waitlist capture exists | `DTD_PROJECT_CONTEXT.md`, `BUILD_CHECKLIST.md`, `LAUNCH_GATE.md` | Owners can join waitlist when live matching is gated | `POST /api/owner-waitlist` persists records; `Home.jsx` renders waitlist-first form when `public_matching_enabled=false` | `docs/COMPLETE_WEBSITE_PAGE_SPEC.md`, `docs/governance/LOCK_STATE.md` | Implemented | Low | Keep as-is | Normal Ops | No |
...
| `/ops` shows enough passive-demand evidence | `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md`, `INITIAL_LAUNCH_EVIDENCE_MODEL_SUPPLY_FIRST.md`, `OPS_COCKPIT_RESPONSIBILITY_MODEL.md` | `/ops` should show volume, trend, duplicates/rejections, suburb demand, and attribution | `/ops` shows waitlist size, joins 24h, duplicate/rejected joins 24h, top suburbs, and 30-day growth totals; current visibility is usable for launch review, though deeper attribution slicing remains limited | `docs/INITIAL_LAUNCH_EVIDENCE_MODEL_SUPPLY_FIRST.md`, `docs/governance/LOCK_STATE.md` | Implemented | Low | Keep as-is; only add richer attribution breakdown if later review needs it | Normal Ops | No |

## 6. Intro Path Readiness Without Public Live Exposure

| Requirement | Source authority | Required runtime behaviour | Current code/runtime evidence | Current docs/evidence reference | Status | Risk if unresolved | Required next action | Suggested owner mode | Blocks runtime/evidence alignment? |
|---|---|---|---|---|---|---|---|---|---|
| Matching path exists as a product capability | `LAUNCH_GATE.md`, `COMPLETE_WEBSITE_PAGE_SPEC.md`, `LOCK_STATE.md` | Owner match path exists even if not currently exposed from home | `/api/match`, trainer detail, intro, engagement, conversion, and follow-up paths exist; `frontend/src/App.js` routes remain active | `docs/governance/LOCK_STATE.md`, `docs/governance/EXECUTION_STATUS.md` | Implemented | Low | Keep as-is | Normal Ops | No |
| Public live exposure is still gated during supply-first | `LOCK_STATE.md`, `LAUNCH_GATE.md`, `INTEGRITY_AUDIT.md` | Matching and contact release should be unavailable from public path when exposure is off | `backend/server.py` calls `_require_public_matching()` for match and intro/contact release; tests assert gated behavior | `docs/governance/LOCK_STATE.md`, `docs/governance/EXECUTION_STATUS.md` | Implemented | Low | Keep as-is | Normal Ops | No |
...
| Requirement | Source authority | Required runtime behaviour | Current code/runtime evidence | Current docs/evidence reference | Status | Risk if unresolved | Required next action | Suggested owner mode | Blocks runtime/evidence alignment? |
|---|---|---|---|---|---|---|---|---|---|
| `/ops` is passcode-gated and bounded to Normal Ops by default | `DTD_PROJECT_CONTEXT.md`, `LAUNCH_GATE.md`, `OPS_COCKPIT_RESPONSIBILITY_MODEL.md` | `/ops` should be protected by oversight auth and remain visibility-first, with only bounded Layer 1 review-state persistence allowed | `POST /api/oversight/login`, `GET /api/oversight`, and `POST /api/oversight/cases/{case_id}` exist; `Ops.jsx` allows queue review-state, owner note, and review-history persistence but no runtime/provider/policy mutation controls | `docs/governance/LOCK_STATE.md`, `docs/governance/OPERATIONS_CONSOLE_SPEC.md` | Implemented | Low | Keep as-is | Normal Ops | No |
| `/ops` stays outside unrestricted admin CRUD | `DTD_PROJECT_CONTEXT.md`, `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md`, `OPS_COCKPIT_RESPONSIBILITY_MODEL.md` | No unrestricted admin surface should be active | `frontend/src/App.js` redirects legacy admin routes; `Ops.jsx` exposes bounded Layer 1 review-state persistence only; no admin CRUD endpoint was found in inspected runtime surfaces | `docs/COMPLETE_WEBSITE_PAGE_SPEC.md`, `docs/governance/LOCK_STATE.md` | Implemented | Low | Keep as-is | Normal Ops | No |
...
| Launch-critical data is sufficient for phase decisions | `LAUNCH_GATE.md`, `INTEGRITY_AUDIT.md`, `INITIAL_LAUNCH_EVIDENCE_MODEL_SUPPLY_FIRST.md` | Product data should be sufficient to support readiness and transition decisions | Launch phase state, readiness snapshots, and phase decision records are now product-backed; local and live `/ops` evidence shows phase, readiness, blockers, and recommendation | Docs define these as required launch-critical records | Implemented | Low | Keep as-is; continue live/provider verification before final owner sign-off | Technical-Owner Mode | No |
...
| Requirement | Source authority | Required runtime behaviour | Current code/runtime evidence | Current docs/evidence reference | Status | Risk if unresolved | Required next action | Suggested owner mode | Blocks runtime/evidence alignment? |
|---|---|---|---|---|---|---|---|---|---|
| No unrestricted admin CRUD | `DTD_PROJECT_CONTEXT.md`, `INITIAL_LAUNCH_GOALS_SUPPLY_FIRST.md`, `LOCK_STATE.md` | Product should not expose admin CRUD as the operating model | Legacy admin routes redirect to `/`; `/ops` exposes bounded Layer 1 review-state persistence only; no active admin CRUD endpoint was found in inspected surfaces | `docs/COMPLETE_WEBSITE_PAGE_SPEC.md`, `docs/governance/LOCK_STATE.md` | Implemented | Low | Keep as-is | Normal Ops | No |
...
| Final supply-first evidence window is complete | `EXECUTION_STATUS.md`, `LOCK_STATE.md`, `LAUNCH_GATE.md` | Final supply-readiness and Go/No-Go evidence should be explicit | Phase/readiness/decision layer is product-backed and test-covered; the current trusted hosted frontend surface and live `/ops` contract are verified; a browser-driven hosted campaign landing and waitlist proof now persists `campaign=phase2-proof` / `source=lp` with `entry_events_30d=3` and `waitlist_joins_30d=1`; a hosted duplicate-path discovery proof moved oversight from `pending=1, duplicate=0` to `pending=0, duplicate=1` and then updated the discovery heartbeat to `duplicates=1`, `handled=1`, `last_run=2026-06-07T18:44:10.935161+00:00`; repeated hosted samples also showed `ranking`, `pricing`, `health`, and `discovery` loop continuity over time; provider-coupled trainer submission and live Stripe/Resend behavior remain intentionally deferred until the final actual-domain activation decision | `docs/governance/LOCK_STATE.md`, `docs/governance/EXECUTION_STATUS.md` | Partial | Medium: owner Go/No-Go remains open and the deferred provider-coupled proof still must be completed at final activation time | Preserve the completed hosted-path evidence set, then perform provider-coupled live checks only at the final actual-domain activation decision before Final Go/No-Go | Owner decision | Yes |

## 14. Implementation Backlog Candidates

| Requirement | Source authority | Required runtime behaviour | Current code/runtime evidence | Current docs/evidence reference | Status | Risk if unresolved | Required next action | Suggested owner mode | Blocks runtime/evidence alignment? |
|---|---|---|---|---|---|---|---|---|---|
| Product-backed launch phase truth | `DTD_PROJECT_CONTEXT.md`, `LAUNCH_GATE.md`, `OPS_COCKPIT_RESPONSIBILITY_MODEL.md` | Runtime needs a separate launch-phase truth source | `PUBLIC_LAUNCH_PHASE` env var, `launch_phase_state` persisted to `system_state`, oversight and `/ops` surface confirmed at `server.py:104, 832-852, 2406` | All major governance docs require it | Implemented | Low | Keep as-is | Normal Ops | No |
...
| Waitlist abnormality visibility | `OPS_COCKPIT_RESPONSIBILITY_MODEL.md`, `INITIAL_LAUNCH_EVIDENCE_MODEL_SUPPLY_FIRST.md` | `/ops` should show duplicate/rejected waitlist signals | `backend/server.py` rolls duplicate/rejected waitlist counts into `owner_waitlist_summary` and creates abnormality cases when those counts are non-zero; `Ops.jsx` overview renders duplicate/rejected 24h counts in the demand-and-supply section | Docs expect monitoring for duplicates/rejections | Implemented | Low | Keep as-is | Normal Ops | No |
...
- none on the current trusted hosted path
...
Next required action: preserve the completed hosted-path evidence set, then perform the deferred provider-coupled live proof only at the actual-domain activation decision before Final Go/No-Go sign-off (`carlg`).
