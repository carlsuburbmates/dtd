...
| Requirement | Source authority | Required runtime behaviour | Current code/runtime evidence | Current docs/evidence reference | Status | Risk if unresolved | Required next action | Suggested owner mode | Blocks runtime/evidence alignment? |
|---|---|---|---|---|---|---|---|---|---|
| `/ops` is passcode-gated and bounded to Normal Ops by default | `SSOT.md`, `LAUNCH_GATE.md`, `OPS_COCKPIT_RESPONSIBILITY_MODEL.md` | `/ops` should be protected by oversight auth and remain visibility-first, with only bounded Layer 1 review-state persistence allowed | `POST /api/oversight/login`, `GET /api/oversight`, and `POST /api/oversight/cases/{case_id}` exist; `Ops.jsx` allows queue review-state, owner note, and review-history persistence but no runtime/provider/policy mutation controls | `docs/governance/LOCK_STATE.md`, `docs/governance/OPERATIONS_CONSOLE_SPEC.md` | Implemented | Low | Keep as-is | Normal Ops | No |
