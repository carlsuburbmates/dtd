...
- **Evidence:** owner-provided Google Cloud Startup guidance; successful Firebase deployment; public HTTP 200 checks for the root, `www`, `robots.txt`, `sitemap.xml` and `/api/health`; authoritative VentraIP queries for apex, `www`, `learn`, Zoho MX, verification, DMARC, DKIM and SES records; commit `e4e8406`.
