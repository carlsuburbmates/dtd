# Design Continuity Index

Updated: 2026-05-07

Start here for future design-system sessions.

## Canonical precedence (highest to lowest)

1. Live implementation files in `frontend/src` and `frontend/src/index.css`
2. `/Users/carlg/Documents/AI-Coding/dtd/frontend/design-tokens/dtd.figma.tokens.json`
3. `/Users/carlg/Documents/AI-Coding/dtd/docs/design/FIGMA_COMPONENT_MAP.md`
4. `/Users/carlg/Documents/AI-Coding/dtd/docs/design/FIGMA_SYNC_PLAYBOOK.md`
5. `/Users/carlg/Documents/AI-Coding/dtd/docs/design/NO_PAYWALL_WORKFLOW.md`
6. `/Users/carlg/Documents/AI-Coding/dtd/docs/design_guidelines.json` (creative direction reference, not canonical runtime truth)

## Documents

1. Sync status and next actions:
- `/Users/carlg/Documents/AI-Coding/dtd/docs/design/FIGMA_SYNC_PLAYBOOK.md`

2. Code-to-Figma component contract:
- `/Users/carlg/Documents/AI-Coding/dtd/docs/design/FIGMA_COMPONENT_MAP.md`

3. Pre-launch no-paywall operating workflow:
- `/Users/carlg/Documents/AI-Coding/dtd/docs/design/NO_PAYWALL_WORKFLOW.md`

## Token file

1. `/Users/carlg/Documents/AI-Coding/dtd/frontend/design-tokens/dtd.figma.tokens.json`

## Figma target

1. https://www.figma.com/design/Is3zJpOaGhUo2nrTFzHww6

## Rule for every future session

If any design decision changes, update all three docs plus the token file in the same session.

## Alignment audit

Latest cross-document alignment check:

- `/Users/carlg/Documents/AI-Coding/dtd/docs/design/DESIGN_ALIGNMENT_REPORT_2026-05-07.md`
