      ...
      {nextToResume && (
        <div className="mb-12">
          <div className="bg-terracotta/5 border border-terracotta/20 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden">
            ...
            <Link 
              to={`/course/${nextToResume.module.id}/${nextToResume.lesson.id}`}
              ...
              <Card className={`group relative overflow-hidden h-full flex flex-col transition-all duration-500 hover:shadow-xl hover:-translate-y-1 ${isLocked ? "opacity-90 bg-muted/20" : "bg-card hover:border-terracotta/50"}`}>
                ...
                <div className="relative z-20 flex flex-col h-full">
                  ...
                  <CardFooter className="mt-auto">
                    {isLocked ? (
                      <Link to={`/course/${module.id}/${module.lessons[0].id}`} className="btn-ghost px-0 text-muted-foreground group-hover:text-white/80 hover:bg-transparent transition-colors duration-300 relative z-30">
                        Unlock Module <Lock className="w-4 h-4 ml-1" />
                      </Link>
                    ) : (
                      <Link to={`/course/${module.id}/${module.lessons[0].id}`} className="btn-ghost px-0 text-terracotta group-hover:text-white hover:text-terracotta-deep group-hover:hover:text-white/80 hover:bg-transparent transition-colors duration-300 relative z-30">
