...
import { Menu, X, CheckCircle2, ChevronRight, Lock } from 'lucide-react';
import confetti from 'canvas-confetti';
import { modules } from '@/data/modules';
import AuthWall from '@/components/education/AuthWall';
...
export default function LessonViewer() {
    ...
    try {
      ...
      setTimeout(() => {
        if (nextModuleId && nextLessonId) {
          navigate(`/the-first-leash/modules/${nextModuleId}/lessons/${nextLessonId}`);
          window.scrollTo(0, 0);
        } else {
          navigate('/the-first-leash/dashboard');
        }
      }, 1200);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        const mod = await import(`@/content/modules/${moduleId}/${lessonId}.mdx`);
        if (isMounted) {
  ...
  const SidebarContent = () => (
    <div className="h-full flex flex-col">
      <div className="p-4 border-b border-border/40">
        <Link to="/the-first-leash/dashboard" className="text-sm font-medium text-muted-foreground hover:text-foreground flex items-center gap-2">
        ...
        <nav className="flex flex-col gap-1">
          {moduleData.lessons.map((lesson) => {
            ...
            return (
              <Link
                key={lesson.id}
                to={`/the-first-leash/modules/${moduleId}/lessons/${lesson.id}`}
          ...
          <nav className="fixed bottom-0 left-0 lg:left-72 right-0 bg-background/95 backdrop-blur-md border-t border-border/40 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] z-30 shadow-[0_-8px_16px_rgba(0,0,0,0.02)]" aria-label="Lesson actions">
            <div className="max-w-3xl mx-auto flex flex-col sm:flex-row gap-4 items-center">
              <button 
                ...
                className={`w-full sm:w-auto flex-1 py-4 px-6 text-lg font-semibold shadow-sm transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 min-h-[56px] rounded-xl flex items-center justify-center gap-2 ${
                  isCompleted 
